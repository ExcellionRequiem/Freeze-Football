...
7. IMPLEMENTACIÓN SHOCKETS
		...
		Está configurado en una clase WebSocketConfig con @Configuration y @EnableWebSocketMessageBroker, declarando los prefijos /client y /server para servidor y cliente (en vez de /topic y /app), cambiando los limites de tamaño de mensaje, buffer y limite de tiempo para enviar un mensaje, y registrando un interceptor para el socket /ws. El interceptor es de tipo HttpHandshakeInterceptor, una clase Java que implementa HandshakeInterceptor y una forma de guardar y obtener el nombre de usuario desde el websocket previamente guardado en la sesión http (fase previa) con uso de un método beforeHandshake @Override.

		Adicionalmente, se ha añadido un método getUsername @ResponseBody y @GetMapping("/api/username") para fetchear el nombre de usuario directamente por rest desde el UserController (utilizado actualmente en el index.html durante la precarga de la escena de menú principal). A través de 4 controladores y 3 clases java para transmitir datos, se sincronizan los jugadores y se realiza el matchmaking: MatchmakingController es utilizado durante la selección de personajes, cuando un usuario ya ha sido suscrito a su ruta personal de matchmaking y utiliza el botón de juego/ready para mandar el comando al controlador mediante /readyGetUser. Utiliza una lista de jugadores que esperan partida y en caso de que haya 2 jugadores o más esperando les devuelve un mensaje para que se metan en partida e identifiquen a su rival. PlayerMovementController, BallMovementController y ScoreController se utilizan durante una partida en si para sincronizar los movimientos de los jugadores, el balón, y el marcador de puntos respectivamente. Todos utilizan un mecanismo similar en el que se designa un usuario host de manera determinista  comparando las cadenas de caracteres de los nombres de usuario, y para la pelota y el marcador solo el host mandará mensajes (a /server/score/{username} y /server/ballsync/{username}) y a su vez solo el rival utiliza estos datos mediante una suscripción a los canales /client/score/{username} y /client/ballsync/{username}, comprobando al enviar un mensaje si el que se lo ha enviado es su rival. PlayerMovementController utiliza el mismo mecanismo pero no distingue entre host y no host, un rival envía al otro y si son rivales mutuos efectúa un cambio con los datos. Utiliza los canales /server/movement/{username} y /client/movement/{username}.

		Las clases usadas como datos en estos controladores, BallData, ScoreMarker y PlayerMovement, tienen estructuras parecidas que esencialmente son los datos que sincronizan (las puntuaciones en el caso del ScoreMarker, las posiciones y las velocidades de phaser en el baso de los otros objetos) y un atributo String "from" que identifica a quién manda el mensaje de Stomp, que es lo que utilizan las suscripciones para saber si el que lo ha mandado es el rival o un jugador de otra partida (también incluyen métodos set y get genéricos estándar de este tipo de clases).
